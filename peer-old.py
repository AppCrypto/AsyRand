...
class Peer(threading.Thread):
    ...
    def start(self):
        ...
        #     #     print(len(node.msgs["initial"][node.curSeq[node.L]]))
            ...
            if sent2 in self.sentDict:           
                # print("%s has sent2 in self.sentDict"%node.id,getattr(node, "id"), sent2, node.epoch, seq, node.newL)
                continue
